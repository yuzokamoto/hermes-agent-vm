...
if pgrep -af 'apt|dpkg|unattended-upgrade' >/dev/null; then
  echo "Package operation is active; do not suspend or snapshot the VM." >&2
...
Review active tmux sessions and Hermes/Kanban workers before suspending VMware or creating a manual snapshot.
